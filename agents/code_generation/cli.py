...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate repository code changes from a SolDesign technical design Markdown."
    )
    parser.add_argument(
        "--design",
        required=True,
        help="Path to SolDesign/Output/technical_design_*.md.",
    )
    parser.add_argument(
        "--repo-path",
    ...
    parser.add_argument(
        "--workspace-dir",
        default=str(Path(__file__).resolve().parents[1] / "solution_design" / ".workspace"),
        help="Existing workspace that contains fetched repositories. Defaults to solution_design/.workspace.",
    )
    parser.add_argument(
        "--task-id",
        help="Optional SolDesign task id if the repository lives in a task-level workspace.",
    )
    parser.add_argument(
        "--output-dir",
