...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "SolDesign workspace repository."
        )
    )
    parser.add_argument(
        "--codegen-result",
    ...
    parser.add_argument(
        ...
        help="Integration runtime workspace. Defaults to Integration/.workspace.",
    )
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Replace an existing Integration task workspace for the same task id.",
    )
    parser.add_argument(
        "--no-commit",
        action="store_true",
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(
            "Missing dependency "
            f"`{missing}`. Install with: python -m pip install -r Integration/requirements.txt"
        ) from exc
    ...
    try:
        ...
        raise SystemExit("Integration interrupted by user.") from None
    except RuntimeError as exc:
        raise SystemExit(f"Integration failed: {exc}") from None
    ...
    print(f"Integration result generated: {result['result_json_path']}")
