...
`ApiServer` 是 DeliveraX 的 FastAPI 编排层。它负责创建 pipeline、调用各阶段统一 `run_stage()` 入口、保存阶段状态，并读取 `artifacts/` 下的标准产物。
...
阶段内部逻辑仍然留在 `ReqAnalysis`、`SolDesign`、`CodeGen`、`Integration` 等模块里。API 层只依赖 `stage_contracts` 和各阶段暴露的 `run_stage(request)`。
