...
const statusConfig: Record<string, { label: string; className: string }> = {
  ...
  pending_approval: { label: "待审核", className: "bg-amber-50 text-amber-600" },
  succeeded: { label: "已完成", className: "bg-emerald-50 text-emerald-600" },
  ...
  terminated: { label: "已终止", className: "bg-slate-100 text-slate-500" },
};
...
export default function PipelineCard({ pipeline, isActive, onClick }: Props) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-full text-left rounded-xl border px-4 py-3.5 transition-all duration-150 cursor-pointer font-sans
        ${isActive
    ...
    >
      ...
      <div className="flex items-center gap-3 mt-2.5 text-[11px] text-slate-400">
        ...
        <span>{pipeline.provider}{pipeline.model ? ` / ${pipeline.model}` : ""}</span>
        <span className="text-slate-300">·</span>
