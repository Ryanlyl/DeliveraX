...
def prepare_integration_repository(
    *,
    ...
    if task_dir.exists():
        if not force:
            raise RuntimeError(
                f"DeliveryIntegration task workspace already exists: {task_dir}. "
...
def _copy_to_synthetic_git_repo(source: Path, repo: Path) -> None:
    ...
    git(repo, ["init"])
    git(repo, ["add", "-A"])
    git(repo, ["commit", "--allow-empty", "-m", "DeliveryIntegration synthetic base"])
...
def _safe_rmtree(target: Path, allowed_root: Path) -> None:
    ...
    if resolved_target == resolved_allowed or resolved_allowed not in resolved_target.parents:
        raise RuntimeError(f"Refusing to remove path outside DeliveryIntegration workspace: {resolved_target}")
