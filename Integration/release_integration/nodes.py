...
def load_prevalidated_changes(state: IntegrationState) -> IntegrationState:
...
def verify_test_and_review_results(state: IntegrationState) -> IntegrationState:
    ...
    if test.get("source") == "assumed":
        state.setdefault("warnings", []).append(
            "No CodeTest result file or status was provided; Integration assumed tests passed by input contract."
        )
    if review.get("source") == "assumed":
        state.setdefault("warnings", []).append(
            "No ReviewGate result file or status was provided; Integration assumed review approval by input contract."
        )
    return state


def resolve_solution_workspace_repo(state: IntegrationState) -> IntegrationState:
    source = Path(state["source_repo_root"]).resolve()
    if not source.exists():
        raise RuntimeError(f"CodeGen workspace repository does not exist: {source}")
...
def create_integration_worktree(state: IntegrationState) -> IntegrationState:
...
def verify_integrated_diff(state: IntegrationState) -> IntegrationState:
...
def generate_delivery_summary(state: IntegrationState) -> IntegrationState:
    ...
    if format_errors:
        raise RuntimeError("Delivery output format validation failed: " + "; ".join(format_errors))
...
def package_merge_ready_output(state: IntegrationState) -> IntegrationState:
...
def _build_summary(state: IntegrationState, final_diff_path: Path, pr_body_path: Path) -> str:
    ...
    lines = [
        ...
        f"- Source workspace repository: `{state.get('source_repo_root', '')}`",
        f"- Integration repository: `{state.get('integration_repo_path', '')}`",
        f"- Integration branch: `{state.get('integration_branch_name', '')}`",
        f"- Base commit: `{state.get('task_base_commit_sha') or ''}`",
        f"- Current commit: `{state.get('head_commit_sha') or ''}`",
        f"- Merge ready: `{state.get('merge_ready', False)}`",
        "",
        "## Upstream Results",
        "",
        "| Stage | Status | Source | Detail |",
        "| --- | --- | --- | --- |",
        f"| CodeTest | {state.get('test', {}).get('status', '')} | {state.get('test', {}).get('source', '')} | {_table_cell(state.get('test', {}).get('detail', ''))} |",
        f"| ReviewGate | {state.get('review', {}).get('status', '')} | {state.get('review', {}).get('source', '')} | {_table_cell(state.get('review', {}).get('detail', ''))} |",
        "",
        "## Integrated Files",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "",
            "```text",
            state.get("final_diff_stat", "").strip() or "No diff statistics are available.",
            "```",
            "",
            "## Output Artifacts",
            "",
            f"- Final diff: `{final_diff_path}`",
            f"- Pull request body: `{pr_body_path}`",
        ]
    )
...
def _build_pr_body(state: IntegrationState, final_diff_path: Path, summary_path: Path) -> str:
    files = state.get("integrated_files", [])
    lines = [
        "## Change Summary",
        "",
        f"Completed delivery integration for `{state.get('task_id', '')}` from a CodeGen diff that passed upstream gates.",
        "",
        "## Upstream Validation",
        "",
        f"- CodeTest: `{state.get('test', {}).get('status', '')}` ({state.get('test', {}).get('source', '')})",
        f"- ReviewGate: `{state.get('review', {}).get('status', '')}` ({state.get('review', {}).get('source', '')})",
        "",
        "## Changed Files",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            f"- Current commit: `{state.get('head_commit_sha') or ''}`",
            f"- Final diff: `{final_diff_path}`",
            f"- Delivery summary: `{summary_path}`",
        ]
    )
    return "\n".join(lines).rstrip() + "\n"
    ...
    *,
    ...
    if not llm.available:
        if state.get("require_llm"):
            raise RuntimeError(
                "LLM summary generation is required, but Integration LLM settings are incomplete. "
                "Set DELIVERY_INTEGRATION_LLM_API_KEY and DELIVERY_INTEGRATION_LLM_MODEL. "
                "For non-default OpenAI-compatible services, also set DELIVERY_INTEGRATION_LLM_BASE_URL."
            )
        state.setdefault("warnings", []).append(
            "Integration LLM is not configured; delivery summary and PR body were generated from templates."
        )
    ...
    try:
        ...
        if not summary or not pr_body:
            raise ValueError("LLM output must include summary_markdown and pr_body_markdown.")
        _assert_valid_delivery_documents(summary, pr_body)
        return summary + "\n", pr_body + "\n", "llm", llm.config.model
    except Exception as exc:
        if state.get("require_llm"):
            raise RuntimeError(f"LLM summary generation was required but failed: {exc}") from exc
        state.setdefault("warnings", []).append(f"LLM summary generation failed; used templates instead: {exc}")
...
def _assert_valid_delivery_documents(summary_markdown: str, pr_body_markdown: str) -> None:
    ...
    if errors:
        raise ValueError("Delivery output format validation failed: " + "; ".join(errors))
...
def _build_result_json(state: IntegrationState) -> dict[str, Any]:
    return {
        "task_id": state.get("task_id"),
